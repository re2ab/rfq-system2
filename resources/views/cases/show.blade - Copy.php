...
<div class="case-tabs-content">
    ...
    <div class="case-row-50">
      <div>
        <x-card title="جزئیات پرونده">
          ...
          <div class="field-row"><span class="lbl">تاریخ ایجاد</span><span class="val">{{ $case->created_at ? jdate($case->created_at, 'Y/m/d') : '—' }}</span><span></span></div>
              ...
              <div class="timeline-item">
                ...
                <div class="timeline-content">
                  <div style="display:flex;justify-content:space-between;gap:8px;flex-wrap:wrap">
                    ...
                    <div class="timeline-meta">{{ jdate($activity->created_at) }}</div>
                  </div>
                  @if($isCall)
                    <div class="call-card">
                      <div class="call-title">گزارش تماس تلفنی</div>
                      @if($activity->call_datetime)
                        <div class="timeline-meta">زمان: {{ jdate($activity->call_datetime) }}</div>
                      @endif
            ...
            <div class="space-y-2 mb-3" style="max-height:280px;overflow:auto">
              @forelse($chatMsgs as $m)
                ...
                <div class="border rounded p-2">
                  <div class="text-xs text-gray-500">{{ $u->name ?? ('#'.$m->user_id) }} · {{ jdate($m->created_at) }}</div>
                  <div>{{ $m->body }}</div>
                </div>
              @empty
  ...
  <div id="tab-documents" class="case-tab-pane" style="display: none;">
    ...
    {{-- ردیف اول: اسناد (راست) + پیوست‌ها (چپ) — ترتیب قبلی برعکس شد
         (درخواست کاربر: جای این دو باکس با هم عوض شود). --}}
    <div class="case-row-50">
      <div>
        <x-card title="اسناد" :pad="false">
          ...
          @php
            // M29 (درخواست کاربر): به‌جای یک ردیف در سطحِ سند، هر رویژنِ
            // منتشرشده‌ی هر سند یک ردیفِ جداست — از نسخه‌ی اصلی (پایین‌ترین
            // revision_number) در ردیفِ اول تا آخرین رویژنِ منتشرشده در
            // آخرین ردیف؛ رویژن‌های پیش‌نویس/در-بررسی (هنوز منتشرنشده) اصلاً
            // نمایش داده نمی‌شوند — هم‌راستا با تصمیمِ قبلی (M24) برای فهرستِ
            // کلیِ اسناد.
            $caseDocRows = collect();
          ...
          @forelse($caseDocRows as $row)
            ...
            <div class="rel-item" style="display:flex;align-items:center;justify-content:space-between;gap:8px">
              <a href="{{ route('documents.show', ['document' => $doc, 'revision' => $rev->id]) }}" class="block" style="flex:1;min-width:0;{{ $isBaseRow ? '' : 'padding-inline-start:15px' }}">
                ...
                <div class="rel-meta">{{ $doc->documentType->name_fa ?? $doc->type }} · {{ $rev->published_at ? jdate($rev->published_at, 'Y/m/d') : ($doc->created_at ? jdate($doc->created_at, 'Y/m/d') : '—') }}</div>
              </a>
              @if($rev->file_path)
        ...
        @forelse(($case->tasks ?? collect())->sortByDesc('id') as $task)
          ...
          <a href="{{ route('tasks.show', $task) }}" class="rel-item block">
            ...
            <div class="rel-meta">
              ...
              @if($task->due_at)
                · سررسید: <span style="{{ $overdue ? 'color:var(--danger);font-weight:700' : '' }}">{{ due_label($task->due_at) }}</span>
              @endif
            </div>
          </a>
        @empty
